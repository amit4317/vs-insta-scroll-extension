...
exports.findChromeWidevine = findChromeWidevine;
exports.launchWithWidevine = launchWithWidevine;
const fs = require("fs");
const os = require("os");
...
function findChromePathFromRegistry() {
    if (!isWindows)
        return null;
    try {
        ...
        for (const key of keys) {
            try {
                ...
                if (match) {
                    ...
                    if (path.basename(dir) === 'Application')
                        return dir;
                }
            }
            catch {
...
function findWidevineInBase(base, tried = []) {
 ...
 */
function findWidevineInUserData(userDataPath, tried = []) {
    ...
    for (const folderName of widevineFolderNames) {
        ...
        const manifestPath = path.join(win64, 'manifest.json');
        const dllPath = path.join(win64, 'widevinecdm.dll');
        if (!fs.existsSync(manifestPath) || !fs.existsSync(dllPath))
            continue;
        try {
            const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    ...
    try {
        ...
        for (const e of entries) {
            ...
            if (!fs.existsSync(path.join(win64, 'widevinecdm.dll')))
                continue;
            const manifestPath = path.join(win64, 'manifest.json');
            if (!fs.existsSync(manifestPath))
                continue;
 ...
 */
function findChromeWidevine(tried = []) {
    if (isWindows) {
    ...
    if (process.platform === 'darwin') {
        ...
        versions.sort((a, b) => compareChromeVersions(b, a));
        for (const ver of versions) {
            const mac64 = path.join(frameworkPath, ver, 'Google Chrome Framework.framework', 'Versions', 'A', 'Libraries', 'WidevineCdm', '_platform_specific', 'mac_x64');
            if (!fs.existsSync(mac64))
                continue;
...
//# sourceMappingURL=widevineLauncher.js.map