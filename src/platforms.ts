...
export interface PlatformConfig {
  id:          PlatformId;
  label:       string;       // shown in sidebar button
  url:         string;       // URL Chrome opens
  profileDir:  string;       // separate Chrome profile per platform
  width:       number;       // viewport DIP width
  height:      number;       // viewport DIP height
  userAgent:   string;       // UA string sent to the site
  snapScript:  string;       // JS injected after load to enter short-form view
  unmuteDelay: number;       // ms to wait before running unmute script
}
