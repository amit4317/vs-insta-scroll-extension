...
function idct8x8(s: Float32Array, out: Float32Array): void {
  ...
  for (let y = 0; y < 8; y++) {
    ...
    tmp[o+0]=p0+q0; tmp[o+7]=p0-q0; tmp[o+1]=p1+r0; tmp[o+6]=p1-r0;
    tmp[o+2]=p2+r2; tmp[o+5]=p2-r2; tmp[o+3]=p3+q3; tmp[o+4]=p3-q3;
  }
  // Column pass
  for (let x = 0; x < 8; x++) {
    ...
    const q0=v4+v7, q1=v5+v6, q2=v5-v6, q3=v4-v7;
    const r0=(q0+q1)*0.7071067812, r2=(q2+q3)*0.7071067812;
    out[x+0 ]=p0+q0; out[x+56]=p0-q0; out[x+8 ]=p1+r0; out[x+48]=p1-r0;
    out[x+16]=p2+r2; out[x+40]=p2-r2; out[x+24]=p3+q3; out[x+32]=p3-q3;
  }
}

/** Decode a baseline JPEG Buffer → flat Uint8Array of R,G,B,R,G,B,... */
function decodeJpeg(buf: Buffer): { pixels: Uint8Array; width: number; height: number } | null {
  try {
    let pos = 0;
    const rd16 = () => { const v = (buf[pos] << 8) | buf[pos+1]; pos += 2; return v; };
    const rdb  = () => buf[pos++];

    if (rd16() !== 0xFFD8) { return null; }   // not a JPEG

    // Quantisation tables [tableId][64]
    const qtables: number[][] = [];
    // Huffman tables [type0=DC/1=AC][id] → {codes, values}
    type HuffTable = { codes: number[]; values: number[] };
    const htables: HuffTable[][] = [[], []];
    // Frame components [id] → {h,v,qtId}
    type CompInfo = { id: number; h: number; v: number; qtId: number; dcPred: number };
    const comps: CompInfo[] = [];
    let width = 0, height = 0;

    // Bitstream state
    let bitBuf = 0, bitCnt = 0;
    let sosData: Buffer | null = null;
    let sosPos = 0;

    const nextBit = (): number => {
      if (bitCnt === 0) {
        let b = sosData![sosPos++];
        if (b === 0xFF) { sosPos++; }   // skip stuffed 0x00
        bitBuf = b; bitCnt = 8;
      }
      const bit = (bitBuf >> 7) & 1;
      bitBuf = (bitBuf << 1) & 0xFF; bitCnt--;
      return bit;
    };
    ...
    const decodeHuff = (ht: HuffTable): number => {
      let code = 0, len = 0;
      for (let i = 0; i < ht.codes.length; i++) {
        code = (code << 1) | nextBit(); len++;
        if (code === ht.codes[i]) { return ht.values[i]; }
      }
      return 0;
    };
    ...
    while (pos < buf.length - 1) {
      ...
      if (marker === 0xDB) {
        // DQT
        while (pos < segEnd) {
          const info = rdb(); const id = info & 0x0F;
          const qt: number[] = [];
          for (let i = 0; i < 64; i++) { qt.push(rdb()); }
          qtables[id] = qt;
        }
      } else if (marker === 0xC0) {
        // SOF0
        rdb(); // precision
        height = rd16(); width = rd16();
        const nc = rdb();
        for (let i = 0; i < nc; i++) {
          const id = rdb(); const sf = rdb(); const qtId = rdb();
        ...
        while (pos < segEnd) {
          ...
          const id    = info & 0xF;
          const counts: number[] = [];
          for (let i = 0; i < 16; i++) { counts.push(rdb()); }
          const total = counts.reduce((a, b) => a + b, 0);
          const values: number[] = [];
          for (let i = 0; i < total; i++) { values.push(rdb()); }
          ...
          // Rebuild properly
          const htCodes: number[] = [];
          code = 0; vi = 0;
          for (let len = 1; len <= 16; len++) {
            for (let k = 0; k < counts[len-1]; k++) {
              htCodes.push(code); code++;
              vi++;
            }
            code <<= 1;
          }
          if (!htables[type]) { htables[type] = []; }
          htables[type][id] = { codes: htCodes, values };
        }
      } else if (marker === 0xDA) {
        // SOS — scan header then raw bitstream
        const ncs = rdb();
        for (let i = 0; i < ncs; i++) { rdb(); rdb(); }
        rdb(); rdb(); rdb(); // Ss, Se, Ah/Al
        // Everything from here to the next non-stuffed 0xFF marker is entropy data
        const scanStart = pos;
        while (pos < buf.length - 1) {
          if (buf[pos] === 0xFF && buf[pos+1] !== 0x00 && buf[pos+1] < 0xD0) { break; }
          pos++;
        }
        sosData = buf.slice(scanStart, pos);
        sosPos = 0; bitBuf = 0; bitCnt = 0;
    ...
    if (!sosData || width === 0 || height === 0) { return null; }
    ...
    for (let row = 0; row < mcuRows; row++) {
      for (let col = 0; col < mcuCols; col++) {
        for (let ci = 0; ci < numComps; ci++) {
          ...
          // Zero remaining
          for (let i = k; i < 64; i++) { blk[ZIGZAG[i]] = 0; }
          ...
          for (let py = 0; py < 8; py++) {
            for (let px = 0; px < 8; px++) {
              ...
              if (ci === 0) { yPlane[pxIdx]  = val; }
              if (ci === 1) { cbPlane[pxIdx] = val; }
              if (ci === 2) { crPlane[pxIdx] = val; }
            }
          }
...
// ═══════════════════════════════════════════════════════════════════════════
// Median-cut dominant color extraction
//
// Classic algorithm: split the color space at the median of the widest channel
// until we have 8 buckets, then pick the largest bucket's average.
// ═══════════════════════════════════════════════════════════════════════════

type RGB = [number, number, number];

function medianCutDominant(pixels: Uint8Array, count: number): RGB {
  ...
  if (all.length < 10) {
    ...
    for (let i = 0; i < count; i++) {
      tr += pixels[i*3]; tg += pixels[i*3+1]; tb += pixels[i*3+2];
    }
    return [tr/count, tg/count, tb/count];
  }

  // Recursive split
  function split(bucket: RGB[], depth: number): RGB[][] {
    if (depth === 0 || bucket.length < 4) { return [bucket]; }
  ...
  let tr=0,tg=0,tb=0;
  for (const [r,g,b] of dominant) { tr+=r; tg+=g; tb+=b; }
...
interface ThemePalette {
  ...
  statusBarFg:      string;   // status bar foreground
  activityBarBg:    string;   // activity bar background
  activityBarFg:    string;   // activity bar foreground icons
  titleBarBg:       string;   // title bar background
  titleBarFg:       string;   // title bar text
  editorCursor:     string;   // cursor color
  selectionBg:      string;   // text selection
  focusBorder:      string;   // focus ring
  editorLineHl:     string;   // current line highlight
}

function buildPalette(dominant: RGB): ThemePalette {
...
export class ChameleonTheme {
  private enabled  = false;
  private sampling = false;          // lock — only one sample in-flight at a time
  ...
  async start(): Promise<void> {
    ...
    const existing = cfg.get<Record<string, string>>(COLOR_CONFIG_KEY) ?? {};
    this.originalColors = Object.fromEntries(
      Object.entries(existing).filter(([, v]) => !v.includes(OUR_MARKER)),
    );
    ...
    this.cdp.on('Runtime.bindingCalled', (params) => {
      if (!this.enabled) { return; }
      if ((params['name'] as string) !== BINDING_NAME) { return; }
  ...
  private async sample(): Promise<void> {
    if (!this.enabled) { return; }
    if (this.sampling) { return; }   // already running — drop this call
    this.sampling = true;
    ...
    try {
      ...
      const b64 = result['data'] as string | undefined;
      if (!b64) { return; }
  ...
  private async applyPalette(p: ThemePalette): Promise<void> {
    const cfg = vscode.workspace.getConfiguration(WORKBENCH_CONFIG);
  ...
  private async restoreColors(): Promise<void> {
    const cfg = vscode.workspace.getConfiguration(WORKBENCH_CONFIG);
